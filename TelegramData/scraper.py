	...
	for l in to_be_processed:
		...
		if l not in done:
			...
			if update != None:
				...
				df_tbp.to_csv(os.path.join(package_dir,'to_be_processed2'))

				df_done = pd.DataFrame(list(done))
				df_done.to_csv(os.path.join(package_dir,'done2'))

				df_groups = pd.DataFrame.from_dict(groups)
				df_groups.to_csv(os.path.join(package_dir,'groups2'))

				df_edges = pd.DataFrame(list(edges.items()), columns = ['destination vertex','origin vertices'])
				df_edges.to_csv(os.path.join(package_dir,'edges2'))
...
async def init(groups, edges: dict, to_be_processed, done):
	...
	df_tbp = pd.DataFrame(list(to_be_processed))
	df_tbp.to_csv(os.path.join(package_dir,'to_be_processed'))
	df_groups = pd.DataFrame.from_dict(groups)
	df_groups.to_csv(os.path.join(package_dir,'groups'))
	df_done = pd.DataFrame(list(done))
	df_done.to_csv(os.path.join(package_dir,'done'))
	df_edges = pd.DataFrame(list(edges.items()), columns = ['destination vertex','origin vertices'])
	df_edges.to_csv(os.path.join(package_dir,'edges'))
